...
static int currUserID,currRoleID,appRoleChoice;
//-------------------------------------end_of_attributes----------------------------------------------------------------------


// Log In Page Sign Up Button Functionality. Brings user from "loginPage" View to "signUpPage" View.
    @FXML
...
// Role IDs: #  ROLE
    ...
    private void signIn(ActionEvent event) throws SQLException, IOException {
    	...
    	if (rs.first()) {
        	...
        	currRoleID = rs.getInt("userRoleType_userRoleTypeID");
    	}
    	if(!rs.isBeforeFirst()){
    	...
    	// Role ID # = 1 (Spectator). Lands on "spectatorLanding.fxml"
    	...
    	// Role ID # = 2 (Operator). Lands on "operatorLanding.fxml"
    	...
    	// Role ID # = 3 (Player). Lands on "leagueOwnerLanding.fxml"
    	...
    	// Role ID # = 4 (Player). Lands on "playerLanding.fxml"
    	...
    	// Role ID # = 5 (Advertiser). Lands on "advertisorLanding.fxml"
    	if((rs.first()) && roleIDCheck == 5){
    		loginPageSignInButton.getScene().getWindow().hide();
...
// Operator Landing Page's Pending Application Button Functionality. Brings User from Operator Landing to Operator Pending User Management Page.
 ...
 // Operator Pending User Management Page Refresh Button Functionality. Populates pending Table View from pending Table. 
    ...
    private void pendingUsersToTableView(ActionEvent event) throws SQLException{
    	...
    	try {
    		ResultSet rs2 = myConnection.createStatement().executeQuery("SELECT appUserID,appUserName,appCurrentRole,roleID_desired FROM arenadatabase.pending");
    		while(rs2.next()) {
    			data.add(new TableViewController(rs2.getInt("appUserID"), rs2.getString("appUserName"), rs2.getInt("appCurrentRole"),rs2.getInt("roleID_desired")));
    		}
    	}
    	catch(SQLException e) {
    		e.printStackTrace();
    	...
    	pendingColumnDesiredRole.setCellValueFactory(new PropertyValueFactory<>("roleID_desire"));
...
// Spectator Page Show / Hide Label(s)
 ...
 // Spectator confirmation YES button Functionality
    ...
    private void confYes(ActionEvent event) throws SQLException, IOException {
    	...
    	if(appRoleChoice == 2) {
    	String confYesInsert = "INSERT INTO pending (roleID_desired,appUserID,appUserName,appUserPassword,appCurrentRole)"
    			+ " VALUES("+appRoleChoice+","+currUserID+","+modCN+","+modCP+","+currRoleID+")";
    	try {
    	...
    	preparedStatement.execute(confYesInsert);
    		...
    		e.printStackTrace();
    		System.out.println("ERROR: AT Controller.confYes.appRoleChoie 3");
    	}
    	finally {
    		myConnection.close();
    	}
    	...
    	if(appRoleChoice == 4) {
    	String confYesInsert ="UPDATE arenadatabase.users SET userRoleType_userRoleTypeID = ? WHERE userID = ?";
    	try {
    	PreparedStatement preparedStatement = myConnection.prepareStatement(confYesInsert);
    	...
    	preparedStatement.executeUpdate();
    		...
    		e.printStackTrace();
    		System.out.println("ERROR: AT Controller.confYes.appRoleChoice 4");
    	}
    	finally {
    		myConnection.close();
    	}
  ...
  // Then Delete that selection from pending Table. 
    ...
    private void approveNewUser(ActionEvent event) throws SQLException {
     ...
     int selectedDesiredRole = userData.getRoleID_desire();
     int selectedUserID = userData.getAppUserID();
     String sqlUpdate ="UPDATE arenadatabase.users SET userRoleType_userRoleTypeID = "+selectedDesiredRole+" WHERE users.userID = "+selectedUserID+" ";
     String sqlUpdateTwo = "DELETE FROM arenadatabase.pending WHERE pending.appUserID ="+selectedUserID+"";
     try {
...
// Operator Manage Users TableView. Populate Table from users Table when user presses Refresh Button. 
        ...
        private void ManageUsersToTableView(ActionEvent event) throws SQLException{
        	...
        	try {
        		ResultSet rs3 = myConnection.createStatement().executeQuery("SELECT userID,userName,userRoleType_userRoleTypeID FROM arenadatabase.users");
        		while(rs3.next()) {
        			data2.add(new ManageUsersController(rs3.getInt("userID"), rs3.getString("userName"), rs3.getInt("userRoleType_userRoleTypeID")));
        		}
        	}
        	catch(SQLException e) {
        		e.printStackTrace();
        	...
        	manageUsersColumnCurrentRole.setCellValueFactory(new PropertyValueFactory<>("userRoleType_userRoleTypeID"));
